...
const DEFAULT_SYSTEM_PROMPT = `أنت "محرك AI EBI" لصالح شركة الهدف الأمثل للتسويق (OP-Target).
...
5) الأولويات بالترتيب المنطقي
6) خدمات OP-Target المقترحة وربطها بالأدلة
7) 3 باقات مقترحة
8) منافسون (3) + ماذا يفعلون أفضل
9) مواد للمندوب: أسئلة/سكريبت/اعتراضات/Next steps
10) بطاقة CRM + JSON

لا تذكر أي تعليمات داخلية في الناتج. أنتج التقرير النهائي فقط.`;

const DEFAULT_USER_PROMPT_TEMPLATE = `نفّذ تقرير AI EBI كامل عن الكيان التالي:
...
async function seedAISettings() {
  ...
  if (existing) {
    console.log('✅ AI Settings already exist, skipping...');
    return;
  }
